package com.meitaomart.app.interceptor;
...
public class LoginInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object arg2) throws Exception {
		...
		if (meitaoResult.getStatus() != 200) {
			...
			//response.sendRedirect("/intercept/login?redirect=" + request.getRequestURL() + "&msg=" + meitaoResult.getMsg() + "&status=" + meitaoResult.getStatus());
			PrintWriter out = response.getWriter();
			
			Map<String, Object> map = new HashMap<>();
	        map.put("redirect", request.getRequestURL());
			meitaoResult.setData(map);
			out.append(JsonUtils.objectToJson(meitaoResult));
			// 拦截
			return false;
		}
