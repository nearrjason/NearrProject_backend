    ...
    <div class="r-filter">
      <div class="f-sort">
        <div class="pagin">
          ...
          <c:if test="${page == 1}">
          	<span class="prev" style="background:url(/images/productList.png) no-repeat  -36px -124px;">
          	    上一页
          	</span>
          </c:if>
          <c:if test="${page > 1}">
          	<span class="prev" style="cursor:pointer;" onclick="searchKeyAndPage('${query}', '${page - 1}')">
          	    上一页
            </span>
          </c:if>
          
          <c:if test="${page == totalPages}">
          	<span class="next" style="background:url(/images/productList.png) no-repeat -54px -124px;cursor:pointer;">
          	    下一页
          	</span>
          </c:if>
          <c:if test="${page < totalPages}">
          	<span class="next" style="cursor:pointer;" onclick="searchKeyAndPage('${query}', '${page + 1}')">
          	    下一页
          	</span>
          </c:if>
    ...
    <div class="p-list">
      <ul class="list-all">
         ...
         <li>
            <div class="l-wrap">
				<div class="pic">
					<%-- <c:set var="imageURI" value="${fn:substring(item.oneImage, 7, -1) }"/> --%>
					<a class="trackref" href="http://192.168.1.100:8086/item/${item.id}.html" title="" target="_blank">
						<img src="${item.oneImage }" style="display:inline"/>
					</a>
				</div>
	            <div class="price">
					<span><span class="p-now">$<strong><fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${item.salePrice / 100 }"/></strong></span><span class="p-nor"><!-- </span><span class="active" style="">直降</span></span>	 -->		
				</div>
	            <div class="title-a">
	                <a class="trackref presaleSign_225865" href="http://192.168.1.100:8086/item/${item.id}.html" target="_blank">${item.name }</a>
	            </div>
	        	<div class="title-b" style=""><a class="trackref" href="http://192.168.1.100:8086/item/${item.id}.html" target="_blank">${sellPoint }</a></div>
	            <!-- <div class="comment">
