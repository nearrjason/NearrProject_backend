...
var E3 = {
    ...
    initPicUpload : function(data){
    	...
    	$("#uploadItemPictureEdit").each(function(i,e){
    		var _ele = $(e);
    		_ele.siblings("div.picItemEdit").remove();
    		_ele.after('\
    			<div class="picItemEdit">\
        			<ul></ul>\
        		</div>');
    		// 回显图片
        	if(data && data.pics){
        		...
        		for(var i in imgs){
        			if($.trim(imgs[i]).length > 0){
        				_ele.siblings(".picItemEdit").find("ul").append("<li><a href='"+imgs[i]+"' target='_blank'><img src='"+imgs[i]+"' width='80' height='50' /></a></li>");
        			}
        		}
        	}
        	//给“上传图片按钮”绑定click事件
        		...
        		KindEditor.editor(E3.kingEditorParams).loadPlugin('multiimage',function(){
        			...
        			editor.plugin.multiImageDialog({
						clickFn : function(urlList) {
							...
							KindEditor.each(urlList, function(i, data) {
								imgArray.push(data.url);
								form.find(".picItemEdit ul").append("<li><a href='"+data.url+"' target='_blank'><img src='"+data.url+"' width='80' height='50' /></a></li>");
							});
							form.find("[name=images]").val(imgArray.join(","));
							editor.hideDialog();
						}
    	...
    	$("#uploadItemDescPictureEdit").each(function(i,e){
    		var _ele = $(e);
    		_ele.siblings("div.picItemDescEdit").remove();
    		_ele.after('\
    			<div class="picItemDescEdit">\
        			<ul></ul>\
        		</div>');
    		// 回显图片
        	if(data && data.pics){
        		...
        		for(var i in imgs){
        			if($.trim(imgs[i]).length > 0){
        				_ele.siblings(".picItemDescEdit").find("ul").append("<li><a href='"+imgs[i]+"' target='_blank'><img src='"+imgs[i]+"' width='80' height='50' /></a></li>");
        			}
        		}
        	}
        	//给“上传图片按钮”绑定click事件
        		...
        		KindEditor.editor(E3.kingEditorParams).loadPlugin('multiimage',function(){
        			...
        			editor.plugin.multiImageDialog({
						clickFn : function(urlList) {
							...
							KindEditor.each(urlList, function(i, data) {
								imgArray.push(data.url);
								form.find(".picItemDescEdit ul").append("<li><a href='"+data.url+"' target='_blank'><img src='"+data.url+"' width='80' height='50' /></a></li>");
							});
							form.find("[name=descImages]").val(imgArray.join(","));
							editor.hideDialog();
						}
